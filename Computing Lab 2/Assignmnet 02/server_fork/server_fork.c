...
{
    int sockfd, ret,valread,str_count;
    ...
    int * cc = mmap(NULL, sizeof(int), PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);
    printf("%d",*cc);
        ...
        *cc += 1;
		...
		if((childpid = fork()) == 0){
			...
			while(1){
                ...
                if ((valread = read( newSocket ,str, MAX)) == 0)
                {
                    printf("Disconnected from %d , ip is : %s , port : %d \n" ,newSocket, inet_ntoa(newAddr.sin_addr) , ntohs(newAddr.sin_port));
                    close(newSocket);
                    break;
                }
                ...
                else if(strcmp(str1,"/similarity")==0 && !regexec(&fileregex, str2, 0 , 0 , 0) && !regexec(&fileregex, str3, 0 , 0 , 0) && str_count == 3)
                {
                    ...
                    if(validateFile(str2) == 0 || validateFile(str3) == 0 ){
                        ...
                        bzero(msg,sizeof(buffer)); 
                        similarityFile(str2,str3,newSocket);
                        bzero(msg,sizeof(buffer)); 
